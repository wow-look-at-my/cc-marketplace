...
function record(sessionId, filePath) {
  const dir = ledgerDir(sessionId);
  if (dir === void 0) return;
  try {
    (0, import_node_fs.writeFileSync)((0, import_node_path.join)(dir, entryName(filePath)), filePath, "utf8");
  } catch {
  }
}
...
function outstanding(sessionId) {
  ...
  if (dir === void 0) return [];
  try {
    return (0, import_node_fs.readdirSync)(dir).map((name) => {
      try {
        return (0, import_node_fs.readFileSync)((0, import_node_path.join)(dir, name), "utf8");
      } catch {
        return "";
      }
    }).filter((path2) => path2 !== "" && (0, import_node_fs.existsSync)(path2));
  } catch {
    return [];
  }
}
...
function sweep(sessionId, cwd) {
  ...
  for (const filePath of outstanding(sessionId)) {
    ...
    const rel = relativePath(filePath, cwd);
    const left = content === void 0 ? [] : findings(rel, content).filter((f) => f.fixable === void 0);
    if (content === void 0 || left.length === 0) {
