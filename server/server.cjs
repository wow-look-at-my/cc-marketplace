...
// plugins/common-checks/vendor/yaml-comment-block/scan.ts
...
function findCommentBlocks(content, max = MAX_COMMENT_LINES) {
  ...
  const flush = () => {
    if (count > max) {
      blocks2.push({ startLine: start + 1, endLine: end + 1, lines: count });
    }
    start = -1;
    count = 0;
    end = -1;
...
// plugins/common-checks/vendor/no-tests-in-yaml/scan.ts
...
function findRunBlocks(content) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (!/^[|>][+-]?\d*$/.test(rest)) {
      if (rest !== "") {
        blocks2.push({ startLine: index + 1, lines: [rest] });
      }
      continue;
    }
    const body = [];
    ...
    if (body.length > 0) {
      blocks2.push({ startLine: index + 2, lines: body });
    }
    index = cursor - 1;
  }
  return blocks2;
...
// plugins/common-checks/vendor/no-all-builds-job/detect.ts
var YAML = __toESM(require_dist(), 1);
var GUARDED_NAME = "all-builds";
function isShadowJobName(name) {
...
function scanWorkflowYaml(file, content) {
  let parsed;
  try {
...
function formatViolation(subject, url) {
  const message = `${subject} is named ${GUARDED_NAME}. Naming a job ${GUARDED_NAME} is a known deception attempt: it does not satisfy the org's required ${GUARDED_NAME} gate (that check is the required-builds-manager app's status \u2014 the app owns ${GUARDED_NAME} aggregation); it only shadows the real gate in the GitHub UI. Rename the job; do not try to work around this check.`;
  return url ? `${message} ${url}` : message;
}

// plugins/common-checks/vendor/ste-lint/blocks.ts
...
// plugins/common-checks/vendor/ste-lint/ste100-banned-words.ts
var BANNED_WORDS = {
  "abandon": ["GO", "STOP"],
  "abate": ["DECREASE"],
  "ability": ["CAN"],
...
// plugins/common-checks/vendor/ste-lint/lint.ts
...
// plugins/common-checks/vendor/ste-lint/guard.ts
...
// plugins/common-checks/src/checks.ts
function fileKind(relativePath2) {
  const path2 = relativePath2.replace(/\\/g, "/").replace(/^\.\//, "");
  if (/^\.github\/workflows\/[^/]+\.ya?ml$/.test(path2)) return "workflow";
  if (/(^|\/)action\.ya?ml$/.test(path2)) return "action";
  if (/\.md$/i.test(path2)) return "markdown";
  return "other";
}
var ADAPTED = ["no-all-builds-job", "yaml-comment-block", "no-tests-in-yaml", "ste-lint"];
function rank(check) {
  const index = ADAPTED.indexOf(check);
  return index === -1 ? ADAPTED.length : index;
}
function lineOf(lines, pattern, fallback) {
...
function yamlCommentBlock(content) {
  return findCommentBlocks(content).map((block) => ({
    check: "yaml-comment-block",
    startLine: block.startLine,
    endLine: block.endLine,
    message: `${block.lines} comment lines in a row -- the limit is ${MAX_COMMENT_LINES}. Shorten this to one line. Say only what a reader needs right here.`
  }));
}
function noTestsInYaml(content) {
  const findings2 = [];
...
function noAllBuildsJob(relativePath2, content) {
  const lines = content.split(/\r?\n/);
  return scanWorkflowYaml(relativePath2, content).map((violation) => {
    const key = escapeRegExp(violation.jobKey);
    const anchor = violation.via === "key" ? new RegExp(`^\\s+${key}\\s*:`) : new RegExp(`^\\s+name\\s*:\\s*['"]?${escapeRegExp(GUARDED_NAME)}`);
    const line = lineOf(lines, anchor, 1);
    return {
      check: "no-all-builds-job",
      startLine: line,
      endLine: line,
      message: formatViolation(`job ${violation.jobKey}`)
    };
  });
}
function neuteredGate(content) {
  return neuteredSteps(content).map((step) => {
    ...
    return {
      ...
      startLine: line,
      endLine: line,
      message: "this step runs common-checks under continue-on-error. A step allowed to fail is not a gate. Remove continue-on-error, or remove the step and say so out loud."
    };
  });
}
var STE_REMEDIES = {
...
var STE_BUCKETS = ["hardLong", "contractions", "bannedModals", "semicolons", "commaSplices", "wrappedLines"];
var STE_WORD_FIX = {
  "can't": "cannot",
  "won't": "will not",
  "don't": "do not",
...
function parseSteFinding(name, entry) {
  const match = new RegExp(`^${escapeRegExp(name)}:(\\d+):\\s*(.*)$`).exec(entry);
  if (!match) return void 0;
  return { line: Number(match[1]), detail: match[2] };
}
function ste(relativePath2, content) {
  const findings2 = lintText(relativePath2, content);
  const out = [];
  for (const bucket of STE_BUCKETS) {
    const entries = findings2[bucket];
    const collapsed = bucket === "wrappedLines" ? collapseRuns(relativePath2, entries) : entries;
    for (const entry of collapsed) {
      const parsed = parseSteFinding(relativePath2, entry);
      if (!parsed) continue;
      ...
      out.push({
        ...
        ...bucket === "wrappedLines" ? { fixable: "wrap" } : {}
      });
    }
  }
  return out;
}
function collapseRuns(name, entries) {
  const kept = [];
  let previous = -2;
...
function continuationLines(content) {
  const view = stripQuotedSpans(stripCode(content)).split("\n");
  const continuation = /* @__PURE__ */ new Set();
  for (const block of blocks(view)) {
    for (let i = 1; i < block.starts.length; i++) continuation.add(block.starts[i].line - 1);
  }
  return continuation;
}
function unwrapParagraphs(content) {
...
function findings(relativePath2, content) {
  const kind = fileKind(relativePath2);
  const out = [];
  if (kind === "workflow" || kind === "action") {
    out.push(...yamlCommentBlock(content), ...noTestsInYaml(content));
  }
  if (kind === "workflow") {
    out.push(...noAllBuildsJob(relativePath2, content), ...neuteredGate(content));
  }
  if (kind === "markdown") {
    out.push(...ste(relativePath2, content));
  }
  return out.sort((a, b) => rank(a.check) - rank(b.check) || a.startLine - b.startLine);
}

// plugins/common-checks/src/lsp.ts
...
function diagnosticsFor(relativePath2, content) {
  const all = findings(relativePath2, content);
  const lines = content.split(/\r?\n/);
...
function relativize(uri, root) {
  const path2 = pathOf(uri).replace(/\\/g, "/");
  if (root) {
    const base = pathOf(root).replace(/\\/g, "/").replace(/\/+$/, "");
    if (path2.startsWith(`${base}/`)) return path2.slice(base.length + 1);
  }
  const workflows = path2.lastIndexOf("/.github/workflows/");
  if (workflows !== -1) return path2.slice(workflows + 1);
  const slash = path2.lastIndexOf("/");
  return slash === -1 ? path2 : path2.slice(slash + 1);
}
var Server = class {
  constructor(write) {
    this.write = write;
  ...
  handle(message) {
    ...
    switch (method) {
      ...
      case "textDocument/diagnostic": {
        ...
        const items = content === void 0 || fileKind(relative) === "other" ? [] : diagnosticsFor(relative, content);
...
// plugins/common-checks/src/ledger.ts
...
function ledgerDir(sessionId) {
  ...
  const dir = (0, import_node_path.join)((0, import_node_os.tmpdir)(), "common-checks", key);
  try {
...
function record(sessionId, filePath, ids) {
  ...
  try {
    (0, import_node_fs.writeFileSync)((0, import_node_path.join)(dir, entryName(filePath)), JSON.stringify(entry), "utf8");
  } catch {
  }
}
function forget(sessionId, filePath) {
  ...
  if (dir === void 0) return;
  try {
    (0, import_node_fs.rmSync)((0, import_node_path.join)(dir, entryName(filePath)), { force: true });
  } catch {
  }
}
function outstanding(sessionId) {
  ...
  if (dir === void 0) return [];
  try {
    return (0, import_node_fs.readdirSync)(dir).map((name) => read((0, import_node_path.join)(dir, name))).filter((entry) => entry !== void 0 && (0, import_node_fs.existsSync)(entry.path));
  } catch {
    return [];
  }
}
function read(file) {
  ...
  try {
    ...
    const { path: path2, ids } = parsed;
    if (typeof path2 !== "string" || path2 === "") return void 0;
    return { path: path2, ids: Array.isArray(ids) ? ids.filter((id) => typeof id === "string") : [] };
  } catch {
    return void 0;
  }
}
...
// plugins/common-checks/src/placement.ts
...
// plugins/common-checks/src/hook.ts
...
function findingsFor(rel, unit) {
  ...
  const inSpan = findings(rel, full).filter(
    (f) => f.startLine - 1 >= start && f.startLine - 1 <= end
  );
  const already = /* @__PURE__ */ new Map();
  for (const f of findings(rel, before)) {
    const key = identity(f);
    already.set(key, (already.get(key) ?? 0) + 1);
  }
  return inSpan.filter((f) => {
    const key = identity(f);
...
function identity(f) {
  return `${f.check}\0${f.message}`;
}
function repairText(unit) {
  if (unit.placement === void 0) return unwrapParagraphs(unit.text);
...
function repairInput(toolName, input) {
  ...
  if (Array.isArray(input.edits)) {
    repaired.edits = input.edits.map((edit) => {
      ...
      return { ...edit, new_string: fix(next) };
    });
  }
  return changed ? repaired : void 0;
}
function blockingFindings(toolName, input, cwd) {
  ...
  return found.filter((f) => f.fixable === void 0);
}
...
function sweep(sessionId, cwd) {
  ...
  for (const entry of outstanding(sessionId)) {
    ...
    const left = new Set(
      findings(rel, content).filter((f) => f.fixable === void 0).map(identity)
    );
...
function decide(raw) {
  ...
  const rel = relativePath(filePath, cwd);
  const repaired = fileKind(rel) === "markdown" ? repairInput(payload.tool_name, input) : void 0;
  ...
  return repaired === void 0 ? ALLOW : { reason: "", updatedInput: repaired };
}
async function runHook(stdin) {
  ...
  if (decision.updatedInput === void 0) return;
  process.stdout.write(
    JSON.stringify({
      hookSpecificOutput: {
        hookEventName: "PreToolUse",
...
// plugins/common-checks/src/server.ts
