...
For rewrites the hook emits `hookSpecificOutput.updatedInput` WITHOUT a `permissionDecision`, so the normal permission flow evaluates the rewritten command (it does not auto-allow); only the heredoc ban emits a `permissionDecision` (`"deny"` + reason). The debug channel is `CLEANUP_BASH_CMDS_LOG`: every rewrite is logged with the rules that fired (`REWRITE ... rules="grep,pipefail"`; the new rules log as `sleep_cap`/`echo_nag`), denies as `DENY ... reason="heredoc"`, and statement-count fail-opens as `GUARD ... reason="stmt-count"`.
