...
**The ledger holds only what THIS session read, and only what it asked GitHub.** Both halves of that were missing. Together they refused the first three GitHub reads of a brand-new session. A resumed or imported conversation writes its earlier records into the same transcript file. A subagent writes its records there too, marked `isSidechain`. Neither is an answer the caller holds. So `parseRecords` takes the payload's `session_id`. It drops a record stamped with another session, and it drops a sidechain record. A record carrying no session id is kept, so a transcript shape without the field behaves as it always did.

**A subject has to sit in the arguments of the GitHub read itself.** `statusStatements` bounds each match to its own statement. The bound runs from the command word to the next separator, and `callText` reads a Bash call's subjects out of those bounds alone. Before that bound, a SHA anywhere in the command text named the subject of whatever `gh` call shared the line. `git show <sha>:src/cmd/go.mod`, `git ls-tree <sha> <path>` and `git log <sha>` were each refused outright. All three read a local object, reach no network, and cost nothing. `git ls-remote` left `statusReadCommands` for the same reason, so no `git` command is a status read at all now.

`commandOf` shares the record walk's `unescapeReplacer` rather than folding only the backslash escapes. A Go encoder escapes an ampersand to its numeric unicode form, and a JavaScript one leaves the character alone. The shorter replacer therefore read `gh pr view 130 && git show <sha>` as one statement. The statement bound then reached the SHA it exists to exclude. That is how the fix looked correct in isolation and failed on a real fixture.
