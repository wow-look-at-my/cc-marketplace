...
- `path` may be a file or a directory; it is whitespace-trimmed and accepts `~` / `~/sub` (expanded to the home directory) like the builtin's path resolution — `~user` is NOT expanded (the builtin didn't either) and null bytes are rejected with `Path contains null bytes`. Missing paths return `Path does not exist: ... Note: your current working directory is ...` with a did-you-mean suggestion when a re-rooted candidate exists.
