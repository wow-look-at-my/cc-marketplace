...
The wording half is `claude_snippets/name-a-thing-link-it.md`. claude-code-web-config shipped its own copy of this rule as `setup/link-what-you-name.ts`. Both fired on the same message, which gave one defect two refusals in different wordings. That copy is retired. This plugin is the only enforcement.
