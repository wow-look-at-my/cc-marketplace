...
`TaskList` and `TaskGet` stay callable while blocked. The list can be checked for duplicates first — but reading the list does **not** settle the debt, or a `TaskList` would buy silence without filing anything. There is deliberately no "declare that there is no task" escape: an escape hatch is the hole the plugin exists to close.
...
The Stop hook honors `stop_hook_active`. Once a stop is already being retried because of a previous block, the hook steps aside. A single firm nudge turns an *accidental* stop into a deliberate one. A stuck session can never hang.
