...
**The digit guard lives in Go, not in the regex. That is not a style choice.** RE2 has no lookbehind, and spelling the guard as a negated class (`[^.\d]`) inside the quantity makes it eat the frame's own separating space. The frame can never meet the quantity -- every positive test went red at once. `continuesANumber` looks at the byte in front of the match instead.
