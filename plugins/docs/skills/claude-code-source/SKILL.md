...
**This is not a preference. A 27 MB, 720k-line bundle must never be read, `cat`ted, `head`ed, or grepped with wide context from the main conversation.** One careless command pastes tens of thousands of tokens of minified JavaScript into the context you need for the actual task. It never comes back out.
