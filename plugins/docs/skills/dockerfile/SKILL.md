...
- **A remote tar (`<src>` is a URL) is NOT extracted by default.** This is the real asymmetry, and the origin of the confused half-memory. To extract it, pass `--unpack=true` (Dockerfile v1.17+).
...
Also: setting `ENTRYPOINT` **resets any `CMD` inherited from the base image** to empty. If the image needs default args, re-declare `CMD` in this Dockerfile. Only the last `ENTRYPOINT`/`CMD` in a stage takes effect.

Need shell features (globs, pipes, `&&`) at runtime? Write an entrypoint script and `ENTRYPOINT ["/entrypoint.sh"]` - and `exec "$@"` at the end of it so the real process becomes PID 1. Alternatively declaring `SHELL ["/bin/bash","-c"]` marks shell form as a conscious choice and suppresses the check.
