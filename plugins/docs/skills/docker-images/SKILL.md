...
Notes to self. `/docs:dockerfile` covers writing the file; this covers deciding what the images should BE. Written after getting it wrong in a real codebase and being corrected by the person who owned it.
...
The rule I should have started from: **layer the program files, mount only the mutable state.** An image is the unit of distribution AND the unit of execution. Splitting those two apart is the smell.
...
`FROM` inherits the base's filesystem AND its configuration. That is the point, and it is also where a copy-based design's habits break when you convert it:
...
This is where the design change actually shows up in a pipeline, and it is easy to get wrong because a matrix looks so tidy:
...
- Does the "carrier" image I am about to write ever run as a container? If not, it should be a stage or a base, not an image.
