...
- **Confirm the size before searching**: a real `cli.js` is ~27 MB / ~720k lines. `gh` exits non-zero on a bad ref (`No commit found for the ref ...`) but still writes its error JSON to stdout, so a failed fetch leaves a ~127-byte file rather than none. `wc -c` once; do not grep a stub.
...
- **Beware the long lines.** The file is prettified, but a formatter cannot break a single token — a few dozen lines are one enormous string or regex literal. `rg -n` prints the whole matched line, so pipe through `cut -c1-200`, prefer `rg -o` with a tight pattern, keep `-C` small, then read the region with Read's `offset`/`limit`.
