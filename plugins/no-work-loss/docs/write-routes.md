...
**The plumbing route**, which never touches the worktree: `hash-object -w`, `update-index`, `commit-tree`, `mktree`, `read-tree`, `checkout-index`, `fast-import`, and `update-ref` when it sets a ref. `update-ref -d` deletes and introduces no content, so it stays with the destruction half, which already asks whether the commits it drops survive elsewhere.
...
- **`echo x > new-file` is refused**, even though nothing is lost. Creating a file with content in it is exactly what Write is for, so the rule is consistent; it is also the refusal a session meets most often.
...
Integrating into a dirty tree is a separate question, and the destruction half answers it: both verbs reach `hazTracked` there and are refused with the `git stash push -u` rewrite, so uncommitted work is still protected.
