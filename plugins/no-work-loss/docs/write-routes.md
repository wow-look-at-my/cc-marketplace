...
A git verb with no write route of its own is tried against `git config`'s own alias table before it is accepted as harmless -- the same `aliasResolver` the destruction half uses. `git nuke` for `reset --hard` writes exactly what the spelled-out verb writes, and this half must see through it or preservation on the destruction side (which does resolve the alias) would let a command escape that the direct spelling still refuses.
