...
`plugin.json` registers the same binary on both `PermissionRequest` and `PreToolUse`. That is not redundancy — each event reaches the tool call at a different point, and only one of them is guaranteed to run.
