...
A properly paced watch loop -- the same check, re-run every 15-30 minutes because a real scheduled trigger fired -- is not this pattern. Spacing, not repetition count, is what tells the two apart: a streak only extends across turns whose gap stays under the max (default 5 minutes). A real gap between two identical checks breaks the streak instead of counting toward it.

A turn that makes no tool call at all -- pure text, waiting -- always resets the streak. Replying with nothing to run is always a valid way to satisfy this hook.
