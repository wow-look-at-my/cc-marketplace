...
- **`hooks/session-heartbeat.ts`** — PostToolUse `*`, async, web sessions only (`CLAUDE_CODE_REMOTE=true`). Sends the bridge's heartbeat action (`session`/`cc`/`ccr`/`cwd`/`tool`, each clamped to the gate's 200-char bound); the server stamps `ts` and applies the fixed 86400s TTL (garbage collection only — liveness is judged from `ts` by the reader). Throttled to one POST per 60s via a marker touched before the network call.
