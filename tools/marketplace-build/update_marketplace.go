...
func buildPluginsArray(plugins []packagedPlugin, existingMarketplace map[string]interface{}, pluginRepo, owner string) []interface{} {
	...
	for _, p := range plugins {
		...
		// They differ in exactly a single way that matters here: the `github`
		// source clones over SSH -- the CLI resolves a github-source entry to
		...
		// which https anonymously is and ssh can never be. A "url" entry clones
		// the given URL verbatim with no SSH branch at all -- correct for a
		// public repo. Verified with no git config, no credential helper and no
		// prompt: `git ls-remote https://github.com/<repo>` lists the plugin
		// tags.
