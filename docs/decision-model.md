...
The invariant is about the content, not about the command. For the ordinary dirty-tree case -- the tracked/untracked/ignored hazard, below -- the hook satisfies the invariant directly instead of refusing: it commits the exact at-risk paths into a dedicated `refs/no-work-loss/<timestamp>.<seq>` ref, through a throwaway `GIT_INDEX_FILE` that never touches the user's own index or working tree, then allows the command. `git read-tree HEAD` seeds that temp index with the last committed tree (skipped when there is no HEAD yet, so the commit gets no parent), `git add --force` stages exactly the paths a denial would have named, and `git commit-tree` builds the commit. A best-effort `git push origin <commit>:<ref>` gets the content off the machine; a push failure still allows, since the local ref already satisfies the invariant on its own, and the one line this is reported on says the push failed and where the content sits instead.

Once that commit exists, the destructive command is safe by construction -- there is nothing left to refuse. The one case that still denies is the commit itself failing: preservation that did not happen must never read as success, so a failed `git add`, `write-tree`, `commit-tree` or `update-ref` falls back to the ordinary denial below.

Two hazard classes are deliberately excluded from this and still deny outright: a stash entry, and anything ref-destroying (the whole "reachability, not refusal" family below). Both ask a different question than "is there uncommitted work", and the complexity of folding a stash pop or a synthesized ref into the same machinery was judged not worth it here -- a stated boundary, pinned by `preserve_test.go`'s negative controls, not an oversight to fix later.

A preservation ref is the ONLY copy of what it holds, so it gets the opposite treatment from every other ref-destroying command: `git update-ref -d` and a force-pushing or deleting `git push` naming one under `refs/no-work-loss/` are refused unconditionally, skipping the "does it exist somewhere else" question entirely -- asking it would trivially and wrongly answer yes, since the ref itself contains it. `git branch -D` cannot reach this prefix at all: a branch name always resolves under `refs/heads/`.

Because destruction and provenance are separate halves (see the provenance section in the top-level CLAUDE.md), a command a provenance rule denies for its own reasons -- `git reset --hard`, `tee`, `truncate`, a truncating redirect -- still ends up denied even after preservation: the preservation commit is a harmless, redundant local artifact, and the notice for it is never surfaced, because the command was not actually allowed to proceed.
